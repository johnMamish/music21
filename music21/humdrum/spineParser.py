...
class HumdrumDataCollection(object):
    ...
    def insertGlobalEvents(self):
        ...
        # it appears that everything in insertList will be a global comment,
        # so it might contain relevant metadata.
        # We should iterate over all of the elements and add them to metadata
        # as appropriate (after making a new metadata for the stream).
        self.stream.insert(0, metadata.Metadata())
        for offset, el in insertList:
            if el.code[:3] == "OTL" :
                self.stream.metadata.title = el.value
        if insertList:
